//https://my.telerikacademy.com/Courses/LectureResources/Video/6112/%d0%92%d0%b8%d0%b4%d0%b5%d0%be-28-%d0%bc%d0%b0%d1%80%d1%82-2015-%d0%95%d0%b2%d0%bb%d0%be%d0%b3%d0%b8

//https://www.youtube.com/watch?time_continue=20&v=rjx2CkZZT08


using System;
using System.Linq;
using System.Collections.Generic;
using DocumentSystemMain;
using DocumentSystemMain.Interfaces;
using DocumentSystemMain.Models;
...
public class DocumentSystem
{
    ...
    private static void EncryptDocument(string name)
    {
        ...
        if (encryptableDocs.Any())
        {
            foreach (IDocument doc in encryptableDocs)
            {
                ...
                {
                    Console.WriteLine("Document does not support enrcyption: {0}", doc.Name);
                }
            }
        }
        else
        {
            Console.WriteLine("Document not dound: {0}", name);
        }
    }
    ...
    private static void DecryptDocument(string name)
    {
        ...
        if (encryptableDocs.Any())
        {
            foreach (IDocument doc in encryptableDocs)
            {
                ...
                {
                    Console.WriteLine("Document does not support dercyption: {0}", doc.Name);
                }
            }
        }
        else
    ...
    {
        var encryptableDocs = documents.Where(d => d is IEncryptable).Select(d => d as IEncryptable);
